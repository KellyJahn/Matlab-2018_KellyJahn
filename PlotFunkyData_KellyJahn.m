...
% At first I used the wrong column, but I didn't want to delete my code :)
% notatall_ind=find(strcmp(data(:, game_header), 'Not at all'));
% fair_ind=find(strcmp(data(:, game_header), 'A fair amount'));
% verymuch_ind=find(strcmp(data(:, game_header), 'Very much'));
% little_ind=find(strcmp(data(:, game_header), 'Just a little'));
% games=NaN(size(data,1), game_header);
% games(notatall_ind)=1; % set not at all to 1
% games(fair_ind)=2;
% games(verymuch_ind)=3;
% games(little_ind)=4;

game_header=find(strcmp(headers,'games_hours'));
subplot(1,3,2);
...
%I am plotting age versus preferred age

f=figure(4);
s=subplot(1,1,1);
age_header = find(strcmp(headers,'age'));
prefage_header = find(strcmp(headers,'pref_age'));
plot([data{:,age_header}], [data{:,prefage_header}], 'g+');
title('How old do you wish you were?');
xlabel('Actual age', 'FontWeight', 'bold');
ylabel('Preferred age', 'FontWeight', 'bold');
set(s,'Color',[0 0 0]);

%basically everyone wants to be in their 20s
